...
午睡時の確認作業を支援するアプリです。iPhone / iPad のカメラを利用して、人の顔が正面かそれ以外かを検知します。正面を向いていたら仰向けと想定して、顔が正面以外は画面上に赤色の枠が顔を囲みます。
...
- iPhone / iPad は、縦向きの利用を想定していますが、横向きでも利用できます。
...
このアプリは、iPhone / iPad のカメラを利用して、人の顔が正面以外を向いているか検知します。アラートが出るのは正面以外の顔だけであり、正確にうつ伏せかどうかを確認できるわけではありませんので、その点には注意が必要です。また、精度は Vision フレームワークに依存するため、使用環境や光の状況によって変動する可能性があります。
